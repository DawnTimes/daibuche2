...
<template>
  <div class="mshbox">
    <div class="msgbox_contant">
      <button type="button" @click="sure()" aria-label="Close" class="el-message-box__headerbtn">
        <i class="el-message-box__close el-icon-close"></i>
      </button>
      <i class="icon iconfont" :class="msgBox.icon"></i>
      <p class="first">{{msgBox.confirst}}</p>
      <p class="last">{{msgBox.consecond}}</p>
      <button
        type="button"
        class="el-button el-button--default el-button--small el-button--primary"
        @click="sure(msgBox.flag)"
      >
        <span>{{msgBox.btncontant}}</span>
      </button>
    </div>
  </div>
</template>
