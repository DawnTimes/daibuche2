...
<el-tabs v-model="editableTabsValue" type="card" @tab-remove="removeTab" @tab-click="tabClick">
  ...
  <el-tab-pane
    ...
    :name="item.name"
  >
    <component :is="componentName" :dataParams="dataParams" v-if="isComponentAlive" @closeTab="removeTab"></component>
  </el-tab-pane>
</el-tabs>  
</template>
<script>
import contractToBeReviewed from '@/components/page/contract/contractToBeReviewed'
import viewTheRepaymentPlan from '@/components/page/contract/viewTheRepaymentPlan'
import approvalContract from '@/components/page/contract/approvalContract'
  ...
  mounted(){
    ...
    methods: {
      ...
      handleOperation(data) {
        ...
        if(flag) {
            let newTabName = data.flag === 'approvalContract'? '合同审批':'查看还款计划'
