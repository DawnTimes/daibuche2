<template>
  <div>
    ...
    <el-row type="flex" class="row-bg" justify="end">
      ...
      <el-button icon="el-icon-plus" type="primary" plain round @click="addCarTypeInfo" v-show= "rolePermission.addCarType">增加车型信息</el-button>
    </el-row>
    <el-table :data="tableData" stripe style size="medium" border  :height="GLOBAL.height" :header-cell-style="{
    'text-align':'center',
    'font-weight':'bold',  
    'background':'#627CAF',    
    'color': '#fff',
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button @click="handleClick(scope.row)" type="text" size="small" v-show= "rolePermission.editCarType">编辑</el-button>
        </template> 
      </el-table-column>
    </el-table>
    <el-pagination
              ...
              <el-col :span="24">
                <div class="grid-content bg-purple">
                  <el-form-item label="品牌" prop='carBrand'>
                    <el-select  :disabled="carModelDisable" v-model="params.carBrand" placeholder="请选择" style="width:100%" @change="carBrandChange" clearable>
                      <el-option
              ...
              <el-col :span="24">
                <div class="grid-content bg-purple-light">
                  <el-form-item label="车系" prop='carSeries'>
                    <el-select  :disabled="carModelDisable" v-model="params.carSeries" placeholder="请选择" style="width:100%" @change="carSeriesChange" clearable>
                      <el-option
              ...
              <el-col :span="24">
                <div class="grid-content bg-purple">
                  <el-form-item label="车型" v-if="this.params.operation !== '3'" prop='carModel'>
                    <el-input v-model="params.carModel" placeholder="请输入" :disabled="carModelDisable"></el-input>
                  </el-form-item>
                  <el-form-item label="车型" v-if="this.params.operation === '3'" prop='carModel'>
                    <el-select v-model="params.carModel" placeholder="请选择" style="width:100%" clearable >
                      <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules:{
          carBrand:[{required: true, message: '请输入车的品牌', trigger: 'blur'}],
          carSeries:[{required: true, message: '请输入车系', trigger: 'blur'}],
          carModel:[{required: true, message: '请输入车型', trigger: 'blur'}],
          carModelName:[{required: true, message: '请输入车型名', trigger: 'blur'}],
      },
    ...
    getCarTypeInfo() {
      ...
      axios.post(url, this.params).then(res => {
        if (res.em === "Success!") {
          ...
          // 车型不唯一
          this.$alert('操作失败，输入车型不唯一，请重新输入！', '提示', {confirmButtonText: '确定'})
        }
      });
    },
    getCarTypeInfo2() {
