      ...
      <el-col :span="8">
        <div class="grid-content bg-purple">
          <el-form-item class="page-form-item" label="经销商简称" prop="agentFullNameViewForSearch">
            <el-select v-model="agentFullNameViewForSearch" placeholder="请选择经销商" ref="selectReport" multiple style="width:100%" @remove-tag="removeTag">
      ...
      <el-col :span="8">
        <div class="grid-content bg-purple-light">
          <el-form-item label="车用途类型" prop="carUse">
            <el-select v-model="form.carUse" placeholder="请选择用途类型" clearable @input="change($event)">
      ...
      <el-col :span="8">
        <div class="grid-content bg-purple">
          <el-form-item label="是否限牌" prop="isLimitLicence">
            <el-select v-model="form.isLimitLicence" placeholder="是否限牌" clearable @input="change($event)">
...
export default {
  ...
  props: ["showMsg", "msgType", "showSearch",'agentFullNameViewForSearchFather','agentCodeArrayFather'],
  name: "carInfoSearch",
  data() {
    return {
      form: {
  ...
  methods: {
    ...
    onSubmit(name) {
      ...
      for(var i = this.form.agentCode.length-1; i>=0; i--) {
        if (!this.form.agentCode[i])
          this.form.agentCode.splice(i,1)
      }
      ...
      console.log(this.agentCodeArray)
      this.$emit("onSearchSubmit", this.form, this.agentFullNameViewForSearch, this.agentCodeArray);      
    },
    onCancel() {
      this.$emit("onSearchSubmit", this.form, this.agentFullNameViewForSearch, this.agentCodeArray); 
      this.$emit("onCancel");
