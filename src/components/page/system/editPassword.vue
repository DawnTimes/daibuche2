    ...
    methods:{
        submit(formName){
          ...
          if (valid) {
                ...
                axios2.post(url, formDatas).then(res => {
                    console.log(res)
                    if (res.data.status === 'SUCCEED') {
                        this.$alert("修改成功!", "提示",{confirmButtonText: '确定'})                  
                    } else {
                        console.log(res.errorMessage)
                        this.$alert(res.data.errorMessage, '提示', {confirmButtonText: '确定'})
                    }
                }).catch(error => {
