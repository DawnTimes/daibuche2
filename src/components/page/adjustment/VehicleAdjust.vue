          ...
          <el-form ref="form" :model="params" label-width="120px">
              ...
              <el-col :span="24">
                <div class="grid-content bg-purple-light">
                  <el-form-item label="车用途类型">
                    ...
                    <el-select
                     clearable
                      v-model="params.carUse"
                      placeholder="请选择新增车型所属车系"
                      style="width:100%"
                    >
            ...
            <el-row>
                ...
                <el-form-item class="page-form-item" label="原经销商名称">
                  <el-select v-model="agentFullNameViewForSearch" placeholder="请选择经销商" multiple ref="selectReport" style="width:91%" @remove-tag="removeTag">
                ...
                <el-form-item class="page-form-item" label="调入经销商">
                  ...
                  <el-form-item label="合同生效日">
                    <el-datePicker
                      ...
                      style="width:85%"
                      v-model="adjustParams.contractValidTime"
                      type="date"
                      placeholder="合同生效日"
                      size="small"
...
/*             licenceCode: "2"
    ...
    provinceChange() {
        let url = common.findCityInfoUrl
        axios.post(url, { provinceCode: this.params.provinceCode }).then(res => {
          if (res.em === "Success!") {
    ...
    doAdjust: function() {
      ...
      if (this.multipleSelection.length  > 0) {
        ...
        if (flag) {
          this.$alert('请确认您所勾选的数据调剂状态都不含预调剂状态！', '提示', {confirmButtonText: '确定'})
          return
        }
      } else {
        this.$alert('请先勾选数据！', '提示', {confirmButtonText: '确定'})
        return
      }      
    ...
    cancelAdjust: function() {
      ...
      if (this.multipleSelection.length  > 0) {
        ...
        if (flag) {
          this.$alert('请确认您所勾选的数据调剂状态全部都为预调剂状态！', '提示', {confirmButtonText: '确定'})
          return
        }
      } else {
        this.$alert('请先勾选数据！', '提示', {confirmButtonText: '确定'})
